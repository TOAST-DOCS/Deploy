<!-- pre-align:aligned sig=cd4aedd91ebd -->
...
* デプロイするサーバーグループを選択できます。サーバーグループは**サーバーグループ**タブで作成できます。

##### 実行サーバー

* **すべてのサーバー**
    * 指定されたサーバーグループのすべてのサーバーにデプロイします。
* **サーバー選択**
    * 指定されたサーバーグループの一部のサーバーを選択してデプロイします。

##### 同時実行数

指定されたサーバーグループ内のサーバーに、同時または個別デプロイします。
...
* デプロイ履歴と履歴別の詳細内容を確認
    * デプロイ結果
    * **結果参照**で、履歴別シナリオ設定内容とタスク別実行結果、デプロイノートを確認できます。
    * 'deploying'状態の時にクリックすると、デプロイ中の状態表示画面に移動します。

デプロイ履歴を実行日およびサーバーグループを基準に検索できます。
...
Auto Scaleサービスのインスタンス拡張に応じてデプロイ設定が行えます。
...
* **Deploy**の下にあるタブから、**デプロイ**> **サーバーグループ作成**をクリックするか、**サーバーグループ**> **新規作成**をクリックします。
...
ClientバイナリのAll、Fixed、Recentバージョンを、希望する方式でデプロイできます。
...
1. **Deploy**の下にある**バイナリグループ**タブで、デプロイするバイナリファイルを選択します。
...
* Pre-run Task ：デプロイ前の処理作業が可能なタスク
    * 対象：サーバーグループに保存しない別途のサーバー指定可
* Normal Task：デプロイプロセスのうち、実行されるタスク
...
* エラー検出およびエラー発生時は、デプロイが中止されます。
    * **デプロイ**タブの**シナリオ実行失敗時**で、**実行中断**が設定されている時
...
* **Resource**タブで、管理するファイルを最新バージョンでデプロイできます。
...
* ターゲットディレクトリ
    * ファイルをデプロイするターゲットディレクトリを指定します。

##### User Command

* ユーザー定義Commandタスクです。
* Available Variables(予約語)を使用できます。
* スクリプトエラー検出およびエラー発生時は、デプロイが中止されます。
    * タブメニューのうち、**デプロイ > シナリオ実行失敗時は実行中断**が設定されている時
