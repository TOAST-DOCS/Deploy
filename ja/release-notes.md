...
* デプロイ実行API Version 2.0追加
    * Auto Scaleサーバーグループにもデプロイの実行が可能
...
* NHN Cloud Instanceをデプロイする際、SSHによるデプロイのほか、NHN Cloud Agentによるデプロイ機能を追加
    * InstanceにFloating IPを割り当てなくてもデプロイ可能
...
#### 機能改善
* CloudTrailサービス連動
    * CloudTrailで、Deployコンソールで発生した**オートスケールデプロイ実行**ユーザーイベントの確認可能
...
#### 機能改善
* デプロイ履歴照会抽出機能を改善
    * pre-run taskのみ存在するデプロイ履歴も含まれるように修正
* Auto Scaleサービス連携デプロイ失敗時にインスタンス廃棄機能を連動
    * Auto Scaleサービスでスケールアウトを進行中にデプロイに失敗すると、スケールアウトしたインスタンスを廃棄
    * 3回以上スケールアウトのデプロイに失敗した場合、スケールアウト機能を停止
* セキュリティ脆弱性パッチ

### 2021. 06. 29.
#### 機能改善
* **デプロイ履歴**タブ検索条件を変更
    * サーバーグループおよび実行日(開始日～終了日)を条件にデプロイ履歴を検索
    * 実行日の検索期間を1年に制限
...
* Auto Scaleサービス連携デプロイ機能を追加(USリージョン除外)
    ...
    * Auto Scaleグループに対するユーザーデプロイ機能
* デプロイ実行確認機能を追加
    * デプロイ実行前にデプロイを実行するかどうかを確認
...
* バイナリグループのリージョン概念を追加
    * バイナリ作成時に指定可能
    * バイナリのダウンロードおよびデプロイ時に、連動するリージョンのストレージからダウンロードができるようになりました。
