...
* 情報照会 API の追加
    * API v2.0、v2.1 では、アーティファクト一覧照会、サーバーグループ一覧照会、バイナリグループ一覧照会、デプロイ履歴照会、バイナリ一覧照会 API を提供します。
...
* デプロイ実行 API Version 2.0 追加
    * Auto Scale サーバーグループでもデプロイを実行できます。

<a id="feature-updates"></a>
#### 機能改善
* Cloud Agent アーティファクトの Command Type でも Auto Scale サーバーグループを作成できるように変更
...
#### 新規機能追加
* デプロイ再進入後のデプロイ終了機能を有効化（デプロイ担当者限定）
...
* デプロイ時の認証方法項目の動作エラーを修正
    ...
    * パスワード/pem 切り替えラジオボタンをクリックしても input タイプが切り替わらない問題を修正しました。
...
* シナリオの Import/Export 機能を追加しました。
* Client Type のメニュー使用制限を解除しました（すべてのメニューが使用可能）。
* サーバーグループの機器区分のための Phase 属性を追加しました。
    * Phase Type が Product であるサーバーのデプロイ時に確認ステップを追加しました。
...
* パスワード入力によるクライアントアプリケーションのバイナリインストールページへのアクセス制御機能を追加しました。
