...
## Dev Tools > Deploy > 使用前の設定ガイド { #dev-tools-deploy-setup-guide-before-use }

この文書では、次のような内容を説明します。

* [サービス使用前の必須事項](/Dev%20Tools/Deploy/ja/setup-guide/#pre-requisites-before-using-the-service)
* [NHN Cloud Agent 使用のための準備](/Dev%20Tools/Deploy/ja/setup-guide/#prepare-to-use-nhn-cloud-agent)
...
> NHN Cloud VM サーバーの場合、SSH接続または NHN Cloud Agent を使用してサーバーへデプロイコマンドを送信します。
> SSH接続の場合は、ターゲットサーバーの IP、ポート、ファイアウォール例外処理などの [SSH 接続のための準備](/Dev%20Tools/Deploy/ja/setup-guide/#prepare-for-an-ssh-connection)が必要です。
> NHN Cloud Agent の場合は、NHN Cloud Agent のインストールや有効性確認などの [NHN Cloud Agent 使用のための準備](/Dev%20Tools/Deploy/ja/setup-guide/#prepare-to-use-nhn-cloud-agent)が必要です。
...
> NHN Cloud VM 以外のサーバーの場合、SSH接続のみでサーバーへのデプロイコマンドを伝達できます。
> デプロイ前にデプロイ対象サーバーとSSHで接続する必要があるため、
> 対象サーバーのIP、ポート、ファイアウォール例外処理などの[SSH接続のための準備](/Dev%20Tools/Deploy/ja/setup-guide/#prepare-for-an-ssh-connection)が必要です。
