<!-- pre-align:aligned sig=7ca503817bac -->

## Dev Tools > Deploy > 概要

Deployサービスを使用すると、'1クリック'で素早くアプリケーションをデプロイできます。

* バイナリファイル
* マルチメディアファイル
* スクリプトファイル
* 実行ファイル
* その他ユーザーが定義したさまざまなファイル 

![[図1]サービス画像](https://static.toastoven.net/prod_tcdeploy/ja/01_ja.png)

<a id="benefits"></a>

## 장점 { #benefits }

* 簡単で便利なバイナリ管理
* 手軽なクライアントバイナリ配信
    * メール
    * テキストメッセージ
* デプロイバージョン別リアルタイムモニタリング
* デプロイ結果メール通知
* デプロイ前後のシナリオ設定
* Webコンソール対応

![02.png](http://static.toastoven.net/prod_tcdeploy/overview/02.png)

<a id="features"></a>

## 提供機能 { #features }

<a id="upload"></a>

### アップロード { #upload }
さまざまなアップロード方式を使用して、簡単にファイルをアップロードできます。
* アップロードインターフェース
    * Webコンソール
    * Jenkinsプラグイン
    * REST API

<a id="download"></a>

### ダウンロード { #download }
ダウンロードリンク通知機能を使用して、アプリを便利にインストールおよびアップデートできます。

<a id="query-binaries"></a>

### バイナリ照会 { #query-binaries }
過去のバイナリデータを保持しておくことで、素早くロールバックできます。

<a id="manage-deployment-projects"></a>

### デプロイプロジェクト管理 { #manage-deployment-projects }
プロジェクトのインフラ情報を連携して、簡単に設定できます。

<a id="manage-execute-deployment-scenario"></a>

### デプロイシナリオ(実行)管理 { #manage-execute-deployment-scenario }
ユーザー定義コマンドを使用して、デプロイ後のサーバー状態を確認できます。

<a id="manage-deployment-history"></a>

### デプロイ履歴管理 { #manage-deployment-history }
デプロイ履歴によりデプロイ内容および以前のバイナリを管理するため、安定性を確保できます。

<a id="glossary"></a>

## 用語説明 { #glossary }

| 用語 | 説明 |
| --- | --- |
| アーティファクト | デプロイを管理する Deploy 構成の基本単位 |
| サーバーグループ | シナリオの実行対象サーバーグループ |
| シナリオ | サーバーで実行するタスクの集合 |
| タスク | デプロイプロセスの実行単位 |
| バイナリ | ユーザーがアップロードしたデプロイ対象ファイル |
| リソース | Webコンソールで作成、修正、履歴管理が可能なデプロイ対象ファイル |
