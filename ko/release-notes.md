...
#### 신규 기능 추가
* API v2.1 시나리오 목록 조회 API 추가
    * 서버 그룹에 매핑된 시나리오의 ID와 이름을 조회할 수 있습니다.
...
#### 신규 기능 추가
* API v2.1 가이드 신규 추가
    * User Access Key 토큰(Bearer) 인증 방식을 적용했습니다.
* 정보 조회 API 추가
    * API v2.0, v2.1에서 아티팩트 목록 조회, 서버 그룹 목록 조회, 바이너리 그룹 목록 조회, 배포 이력 조회, 바이너리 목록 조회 API를 제공합니다.
...
* 배포 실행 API Version 2.0 추가
    * Auto Scale 서버 그룹도 배포를 실행할 수 있습니다.
...
* Jenkins Plugin 업데이트(버전 1.1.3)
    * 바이너리 그룹 키가 빈 값인 경우 Default 바이너리 그룹에 업로드되도록 수정했습니다.
    * 엔드포인트의 기본 값이 https://api-tcd.cloud.toast.com에서 https://api-tcd.nhncloudservice.com으로 변경되었습니다.
...
* 클라이언트 바이너리 배포 UI 개선
    * 알림 수신 그룹으로도 다운로드 경로를 전송할 수 있습니다.
...
* NHN Cloud Instance 배포 시 SSH를 통한 배포 외 NHN Cloud Agent를 통한 배포 기능 추가
    * Instance에 Floating IP를 할당하지 않아도 배포할 수 있습니다.
...
* 알림 메일 수신 대상 설정 기능 추가
    * 조직/프로젝트 대시보드 > 알림 관리에서 수신 메일 주소명을 설정할 수 있도록 기능을 추가했습니다.
...
#### 기능 개선
* Jenkins Plugin 업데이트(버전 1.1.2)
    * Windows 환경에 설치된 Jenkins에서 Deploy로 업로드되지 않는 오류를 수정했습니다.
...
#### 기능 개선
* Jenkins Plugin 업데이트(버전 1.1.1)
    * Jenkins Agent 노드에서 Deploy로 업로드되지 않는 오류를 수정했습니다.
...
#### 기능 개선
* CloudTrail 서비스 연동
    * CloudTrail에서 Deploy 콘솔에서 발생한 **오토스케일 배포 실행** 사용자 이벤트를 확인할 수 있습니다.
...
#### 기능 개선
* 배포 이력 조회 추출 기능 개선
    * pre-run task만 존재하는 배포 이력도 포함되도록 수정했습니다.
* Auto Scale 서비스 연계 배포 실패 시 인스턴스 폐기 기능 연동
    * Auto Scale 서비스에서 스케일 아웃을 진행하다 배포에 실패하면 스케일 아웃된 인스턴스를 폐기합니다.
    * 3회 이상 스케일 아웃 배포 실패 시 스케일 아웃 기능을 중지합니다.
* 보안 취약점 패치
...
#### 기능 개선
* **배포 이력** 탭 검색 조건 변경
    * 서버 그룹 및 실행일(시작일~종료일) 조건으로 배포 이력을 검색합니다.
    * 실행일 총 검색 기간을 1년으로 제한합니다.
* 배포 이력 결과 Excel 파일로 다운로드 기능 추가
    * 바이너리 파일이 없는 경우 제외하여 다운로드할 수 있는 선택 옵션을 추가했습니다.
...
* Auto Scale 서비스 연계 배포 기능 추가(US 리전 제외)
    * Auto Scale 타입의 서버 그룹 생성 및 시나리오 매핑 기능을 제공합니다.
    * Auto Scale 그룹에 대한 사용자 배포 기능을 제공합니다.
* 배포 실행 확인 기능 추가
    * 배포 실행 전에 배포 실행 여부를 확인합니다.
...
#### 기능 개선
* TOAST Trail 서비스 연동
    * TOAST Trail에서 Deploy 콘솔에서 발생한 사용자 이벤트를 확인할 수 있습니다.
...
* 아티팩트 생성 시 기본 바이너리 그룹 리전 설정 기능 추가
    * 아티팩트 생성 시 바이너리 그룹 리전을 지정할 수 있습니다(KR1/JP1).
* 아티팩트 설정 시 기본 바이너리 그룹 지정 기능 추가
    * 아티팩트 내 바이너리 그룹 중 선택할 수 있습니다.
...
* 바이너리 그룹의 리전 추가
    * 바이너리 생성 시 지정합니다.
    * 바이너리 다운로드 및 배포 시 연동되는 리전의 스토리지에서 다운로드합니다.
...
#### 기능 개선
* 배포 이력 탭 > 결과 보기 팝업 > 배포 진행 상황이 실시간 업데이트되도록 개선
...
#### 오류 수정
* 서버 그룹 수정 시 서버 그룹 전체 선택 체크박스 오작동 현상 수정
...
* 배포 시 인증 방법 항목 동작 오류 수정
    * pem 파일 필수값 체크를 추가했습니다.
    * 패스워드/pem 전환 라디오 버튼 클릭 시 input 타입이 전환되지 않는 문제를 수정했습니다.
...
* 서버 그룹 장비 구분을 위한 Phase 속성 추가
    * Phase Type이 Product인 서버의 배포 시 확인 단계를 추가했습니다.
...
* 비밀번호 입력을 통한 클라이언트 애플리케이션의 바이너리 설치 페이지 접근 제어 기능 추가
