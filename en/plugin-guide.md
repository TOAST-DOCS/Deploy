...
You can use the NHN Cloud Deploy Jenkins upload plugin to upload Jenkins build artifacts to the NHN Cloud Deploy server.

<a id="integrating-deploy---jenkins"></a>

## Deploy and Jenkins integration procedure { #integrating-deploy---jenkins }

Jenkins build -> Upload binary to Deploy (by plugin) -> Run deployment scenario (deploy/stop/restart/other pre-tasks and post-processing)
To do this, you must set up the deployment environment in the following order:

1. Create an artifact in Deploy
2. Configure a Jenkins build job
3. Create a server group
4. Create a deployment scenario

<a id="installing-jenkins"></a>

## Install Jenkins { #installing-jenkins }

For more information about installing Jenkins, see [https://jenkins.io/](https://jenkins.io/).
...
A predefined profile that helps users run build commands using Jenkins-CLI.
