...
* This API runs a deployment.
* The deployment execution API is provided only when the artifact's `Command Type` is Cloud Agent. (Not supported for SSH.)
* In v2.0, you can also run deployments on Autoscale server groups.
...
* API to get a list of binary groups belonging to an artifact.
...
* API to query the deployment history of an artifact.
...
* This API views the list of binaries that belong to a binary group.
