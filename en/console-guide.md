...
1. On the **Deploy** screen, click **Server Group > New** in the tab at the bottom.
2. In the **Create Server Group** window, configure the server group to create.
    * Enter name (required) and description (optional).
    * Select an OS and specify a Shell Type. You can select a Shell Type from the **Shell Type** list or enter one manually.
    * Select Phase: Choose a server tool. Otherwise, select NONE.
    * Add servers
        * There are two ways to add servers, as described in detail in [Detail Functional Guide Server Group Menu](/Dev%20Tools/Deploy/en/reference/#server-group):
            * Bulk add
            * Individual add
...
3. After completing the entries, click **Create**.
...
A task is a scenario component that performs individual functions and controls their execution order.
There are two types of tasks:

* pre-run Task: Execute before deployment
* Normal Task: Execute while deployed

You can select and use whichever you need. This section covers the tasks required for basic deployment.
...
* You can use Available Variables.
    * Available Variables: Reserved words. You can find more information in the [Tasks menu of the Detail Functional Guide](/Dev%20Tools/Deploy/en/reference/#tasks).
